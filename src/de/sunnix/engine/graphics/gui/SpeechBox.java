...
import de.sunnix.engine.GlobalConfig;
import de.sunnix.engine.InputManager;
import de.sunnix.engine.graphics.Camera;
import de.sunnix.engine.graphics.gui.text.Text;

import java.util.Arrays;

import static de.sunnix.engine.graphics.gui.text.Text.*;
...
public class SpeechBox {
    ...
    private final Text first_line = new Text(false, tc -> tc.setColor(textColor[0], textColor[1], textColor[2], textColor[3]).setExceptionalColoringChars(ARROW_RIGHT, CURSOR_3 + 1).setDrawShadow(false)).setSize(16);
    private final Text second_line = new Text(false, tc -> tc.setColor(textColor[0], textColor[1], textColor[2], textColor[3]).setExceptionalColoringChars(ARROW_RIGHT, CURSOR_3 + 1).setDrawShadow(false)).setSize(16);
    private final Text third_line = new Text(false, tc -> tc.setColor(textColor[0], textColor[1], textColor[2], textColor[3]).setExceptionalColoringChars(ARROW_RIGHT, CURSOR_3 + 1).setDrawShadow(false)).setSize(16);
    private final Text text_arrow = new Text(false, tc -> tc.setText(Character.toString(CURSOR_0)).setExceptionalColoringChars(CURSOR_0, CURSOR_3 + 1)).setSize(16);
    private final Text name_line = new Text(false, tc -> tc.setColor(textColor[0], textColor[1], textColor[2], textColor[3]).setExceptionalColoringChars(ARROW_RIGHT, CURSOR_3 + 1).setDrawShadow(false)).setSize(16);
    ...
    public SpeechBox() {
        first_line.setPos(38, 270 + scroll.getOffsetY());
        ...
        text_arrow.setPos(Camera.getSize().x - 40, third_line.getPos().y - 10 - scroll.getOffsetY());
    }
    ...
    public void render(){
        if(visible) {
            ...
            if(name != null){
                ...
                name_line.setPos(currentNameBox.getPos().x + currentNameBox.getSize().x / 2 - name_line.getWidth() / 2 + currentNameBox.getOffsetX(), currentNameBox.getPos().y + 130);
            }
            if(name != null)
                name_line.render();
            if(action != FADE_IN)
